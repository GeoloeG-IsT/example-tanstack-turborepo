...
import { LANGUAGES, MODELS, type Language, type Model } from '@tanstack-turborepo/shared';
		...
		<div className="min-h-screen bg-zinc-950 text-white font-sans flex justify-center">
			<div className="flex flex-col gap-8 max-w-3xl w-full p-8 pt-16">
				<header className="flex flex-col items-center gap-2 text-center mb-8">
					...
					<h1 className="text-4xl font-extralight">Agentuity + TanStack + Turborepo</h1>
					<p className="text-zinc-400 text-lg">Type-safe AI translation with monorepo architecture</p>
				</header>
				...
				<div className="bg-black border border-zinc-800 rounded-lg p-8 shadow-2xl flex flex-col gap-6">
					...
					<div className="flex flex-col gap-4">
						...
						<button
							className="bg-gradient-to-r from-cyan-600 to-blue-600 text-white font-medium py-3 px-6 rounded-md hover:from-cyan-500 hover:to-blue-500 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
					...
					<div className={`bg-zinc-900 border border-zinc-700 rounded-md p-4 font-mono ${!data ? 'text-zinc-500' : 'text-cyan-300'}`}>
						{data ? (
					...
					<div className="bg-black border border-zinc-800 rounded-lg p-6">
						...
						<div className="flex flex-col gap-3">
							{data.history.map((entry) => (
								<div key={`${entry.toLanguage}-${entry.model}-${entry.text}`} className="bg-zinc-900 border border-zinc-800 rounded-md p-3 text-sm">
